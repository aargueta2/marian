...
void NthElement::getNBestList(const std::vector<uint>& beamSizes, mblas::Matrix& Probs,
  ...
  */
  ...
  getNBestList(Probs, batchFirstElementIdxs, cummulatedBeamSizes, custom_beam_size);
...
{
#if DEBUG
  std::cout <<  "GET THE NBEST" << std::endl;
#endif
  const uint vocabSize = probs.dim(1);
...
#if DEBUG
  std::cout << "- " << d_batchPosition.size() << " - " << d_cumBeamSizes.size() << std::endl;
#endif
  ...
  */
#if DEBUG
  std::cout <<  "END GET THE NBEST" << std::endl;
#endif
}
...
{
#if DEBUG
  cout << "CALLING CUSTOM NBEST " << endl;
#endif
  const uint vocabSize = probs.dim(1);
  const uint numBlocks = uint(custom_beam_size * vocabSize / (2 * BLOCK_SIZE)) + uint(custom_beam_size * vocabSize % (2 * BLOCK_SIZE) != 0);
  const uint numBatches = batchFirstElementIdxs.size() - 1;
...
void NthElement::getValueByKey(std::vector<float>& out, const mblas::Matrix &d_in) const
{
  ...
  HANDLE_ERROR( cudaStreamSynchronize(mblas::CudaStreamHandler::GetStream()));
#if DEBUG
  cout << "END CALL VALUE BY KEY" << endl;
#endif
}

void NthElement::getValueByKey(std::vector<float>& out, const mblas::Matrix &d_in, uint custom_beam_size) const
{
  // need a model with multiple scorers to test this method
  assert(false);
